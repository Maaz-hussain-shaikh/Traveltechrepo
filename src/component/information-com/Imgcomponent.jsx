...
const Imgcomponent = ({ updateParentState ,imgurl}) => {
  const galleryItems =Object.entries(imgurl)
        ...
        {galleryItems.map((item, index) => (
          <section key={index} className="relative flex-shrink-0 w-full h-full overflow-hidden rounded-lg shadow-xs cursor-pointer hover:shadow-md">
            <div className="flex flex-col sm:flex-row">
              ...
              <img src={galleryItems[index][1]} alt="gal" className="object-cover w-full h-32 transition duration-500 transform sm:h-56 hover:scale-110 bg-subtitle" />
            </div>
          </section>
        ))}
      </div>
